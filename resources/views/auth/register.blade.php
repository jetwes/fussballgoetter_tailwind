<x-layouts::app title="Registrieren">
    <x-guest-card title="Neues Konto anlegen">
        <form method="POST" action="{{ route('register') }}" class="space-y-6">
            ...
            <flux:input label="Name" name="name" :value="old('name')" required autofocus autocomplete="name" description:trailing="So wirst du in der Teilnehmerliste angezeigt." />

            <flux:input label="E-Mail-Adresse" type="email" name="email" :value="old('email')" required autocomplete="email" />

            <flux:input label="Passwort" type="password" name="password" required autocomplete="new-password" viewable description:trailing="Mindestens 8 Zeichen." />
