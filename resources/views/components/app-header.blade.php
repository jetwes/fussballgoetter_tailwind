<flux:header class="border-b border-zinc-200 bg-white px-0! dark:border-zinc-700 dark:bg-zinc-800">
    ...
    <a href="{{ route('home') }}" class="flex items-center" wire:navigate.hover>
    ...
    <flux:dropdown align="end">
        <flux:profile :avatar="auth()->user()->avatar_url" :name="auth()->user()->name" circle class="max-sm:[&_[data-slot=name]]:hidden" />

        <flux:menu>
            <flux:menu.heading>{{ auth()->user()->email }}</flux:menu.heading>
            <flux:menu.item href="{{ route('profile') }}" icon="user-circle">Profil bearbeiten</flux:menu.item>
            <flux:menu.separator />
